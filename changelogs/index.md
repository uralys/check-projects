# Changelogs

All notable changes to check-projects will be listed here.

## Versions

- [v1.2.19](./1.2.19.md) - 2026-06-08 - patch release
- [v1.2.18](./1.2.18.md) - 2026-06-08 - patch release
- [v1.2.17](./1.2.17.md) - 2026-06-08 - Revamped `make tag` workflow
- [v1.2.16](./1.2.16.md) - 2026-06-08 - Pull non-current branches through a temporary worktree
- [v1.2.15](./1.2.15.md) - 2026-03-30 - Pull behind branches after status report & `make tag`
- [v1.2.14](./1.2.14.md) - 2026-02-17 - `--update` flag to install new releases on demand
- [v1.2.13](./1.2.13.md) - 2026-02-17 - Symlinked projects support
- [v1.2.12](./1.2.12.md) - 2025-12-22 - Non-blocking update check at startup
- [v1.2.11](./1.2.11.md) - 2025-12-22 - Configurable fetch concurrency
- [v1.2.10](./1.2.10.md) - 2025-12-22 - CLI fetch progress bar
- [v1.2.5](./1.2.5.md) - 2025-11-24 - Remote branch tracking & automatic fetching in TUI
- [v1.2.4](./1.2.4.md) - 2025-11-22 - TUI category selection fixes & UI improvements
- [v1.2.3](./1.2.3.md) - 2025-11-21 - TUI bug fixes & staged changes detection
- [v1.2.0](./1.2.0.md) - 2025-11-21 - Remote status detection & new config options

## Format

Each changelog includes:

- **Bug Fixes**: Issues resolved
- **Features**: New functionality added
- **Breaking Changes**: Changes requiring user action
- **Improvements**: Performance or UX enhancements
- **Technical Details**: Implementation specifics
